...
size class to enter the next size class (excluding the largest fish). The
growth flux out of a size class $i$ is 
\begin{equation}
J_{\mathrm{out},i} = \gamma_i B_i,
\end{equation}
where $\gamma_i$ is the growth rate (year$^{-1}$). The growth
flux to a size class includes two part for the smallest fish and
...
\begin{equation}
C_{max,i} = k_{\mathrm{T}} \cdot a_{c} \cdot m_{i}^{b_{c}}  \label{eq:maxconsump}
\end{equation}
\begin{equation}
V_{i} = k_{\mathrm{T}} \cdot a_{e} \cdot m_{i}^{b_{e}}      \label{eq:clearance}
\end{equation}
\begin{equation}
M_i = k_{\mathrm{TM}} \cdot a_{m} \cdot m_{i}^{b_{m}}       \label{eq:metabolism}
\end{equation}
Here $k_{\mathrm{T}}$ and $k_{\mathrm{TM}}$
are scaling factors of temperature effects (see Section **Temperature
effects**). $a_{c}$, $a_{e}$, and $a_{m}$ are factors for each
biological rate and $b_{c}$, $b_{e}$, $b_{m}$ are exponents.
...
\begin{equation}
\psi_i = \left(1+\left(\frac{m_{c,i}}{m_{\mathrm{mature}}}\right)^{-5} \right)^{-1}
\end{equation}
...
\begin{equation}
m_{\mathrm{mature}} = \eta_{\mathrm{mature}} w_{\mathrm{max}}
\end{equation}
...
The second core element of FEISTY, besides the equation for biomass dynamics, is the specification of how each size class interacts via predation. This is specified in the feeding preference matrix (also known as a food-web matrix) $\theta_{i,j}$ where each element is a number between 0 and 1 which says how much fish class $i$ preys on class $j$. The feeding preference function is either based on size preference or derived from size preference coupling with the vertical habitat of each functional group. All of them are combined with additional modifications to account for feeding specializations.
...
\end{equation}
The calculation of $\theta_{\mathrm{s},i,j}$ follows Eq. \ref{eq:lognormalpreference} in setupVertical and Eq. \ref{eq:errorpreference} in setupVertical2. The detailed description of $\theta_{\mathrm{v},i,j}$ in the Section **Vertical overlap**.
...
Width of vertical distribution of each organism $\omega_i$ including all resources and fish. 
...
Ultimately, the total vertical overlap is the averaged value of day and night vertical distribution values.
...
Zooplankton resources (small and large mesozooplankton) are supposed to be divide into two types in terms of whether they can conduct the diel vertical migration if water column conditions allows. During daytime, a half of them moves to deep water to avoid predation ($z_{\mathrm{loc},\mathrm{day},i} = z_{\mathrm{dvm}}$). Another half is in the surface water ($z_{\mathrm{loc},\mathrm{day},i} = 0$). At nighttime they stay at the surface ($z_{\mathrm{loc},\mathrm{x},i} = 0$).
...
**Small pelagics**\
They always stay at in the surface water ($z_{\mathrm{loc},\mathrm{day},i} = z_{\mathrm{loc},\mathrm{night},i} = z_{\mathrm{bottom}}$).
...
**Large pelagics**\
During daytime, Large pelagics perform differently according to their size. The small- and medium-size classes are at surface ($z_{\mathrm{loc},\mathrm{day},i} = 0$). The large-size classes are divided into two groups: half of them are always in the surface water ($z_{\mathrm{loc},\mathrm{day},i} = 0$), and half in midwater ($z_{\mathrm{loc},\mathrm{day},i} = z_{\mathrm{dvm}}$). 
They all move to surface during night ($z_{\mathrm{loc},\mathrm{night},i} = 0$)

**Bathpelagics**\
All bathypelagic fish stay at the dvm depth during daytime ($z_{\mathrm{loc},\mathrm{day},i} = z_{\mathrm{dvm}}$). At night, the small- and medium-size classes are always at surface ($z_{\mathrm{loc},\mathrm{night},i} = 0$). Large-size classes are in midwater ($z_{\mathrm{loc},\mathrm{night},i} = z_{\mathrm{dvm}}$).

**Demersals**\
During daytime, small demersals are at the surface ($z_{\mathrm{loc},\mathrm{day},i} = 0$); medium demersals are at bottom ($z_{\mathrm{loc},\mathrm{day},i} = z_{\mathrm{bottom}}$); large demersals are at midwater ($z_{\mathrm{loc},\mathrm{day},i} = z_{\mathrm{dvmdem}}$).
At night, small demersal fish are at surface ($z_{\mathrm{loc},\mathrm{night},i} = 0$). Medium and large demersal fish are at the bottom ($z_{\mathrm{loc},\mathrm{night},i} = z_{\mathrm{bottom}}$).

\begin{table}[htbp]

\centering
\caption{Main parameters of FEISTY $z_{\mathrm{loc},\mathrm{x},i}$}
...
Resources\\
\hline
Zooplankton & \multicolumn{3}{l}{0 and $z_{\mathrm{dvm}}$} & \multicolumn{3}{l}{0} \\
...
Demersal fish & 0 & $z_{\mathrm{bottom}}$ & $z_{\mathrm{dvmdem}}$ & 0 & $z_{\mathrm{bottom}}$ & $z_{\mathrm{bottom}}$ \\\
&&&($z_{\mathrm{dvmdem}}$ and $z_{\mathrm{bottom}}$)$^e$&&&($z_{\mathrm{dvmdem}}$ and $z_{\mathrm{bottom}}$)$^e$\\

\hline
\multicolumn{7}{l}{\footnotesize $^a$ ...} \\
\multicolumn{7}{l}{\footnotesize $^b$ ....}\\
\multicolumn{7}{l}{\footnotesize $^e$ if the water column is shallower than the photic zone depth ($z_{\mathrm{bottom}} < z_{\mathrm{photic}}$),}\\
\multicolumn{7}{l}{\footnotesize large demersal fish migrate over the water column day and night (here $z_{\mathrm{dvmdem}}=0$).}
...
Large pelagic fish have reduced feeding preference on forage fish ($\theta_{\mathrm{A}} \cdot \theta_{i,j}$).
...
\begin{equation}
m_{\mathrm{fishing}}=\eta_{\mathrm{F}} w_{\mathrm{max}}
\end{equation}
where $\eta_{\mathrm{F}}$ controls the weight of fish with 50% harvesting rate relative to the max weight of the functional type.
...
To simplify the temperature input, three temperature inputs ($T_{\mathrm{p}}$, $T_{\mathrm{m}}$, $T_{\mathrm{b}}$) are required rather than the water column temperature profile in setupVertical2. Effective temperature of different size classes of each functional type are the averaged values of temperatures of their approximate vertical positions of day ($T_{\mathrm{day}}$) and night ($T_{\mathrm{night}}$).
\begin{equation}
T_{\mathrm{e}} = \frac{T_{\mathrm{day}}+T_{\mathrm{night}}}{2}
\end{equation}

Note this $T_{\mathrm{e}}$ is different from the one in the Section **Real-time effective temperature**. The implementation is hard coded in the function `setupVertical2()`. 
...
\begin{equation}
 T_{\mathrm{day}} =
  \begin{cases}
     T_{\mathrm{m}}, & \text{$ dvm \neq bottom $ and $ dvm \neq 0 $ } \\
     T_{\mathrm{b}}, & \text{$ dvm = bottom $} \\
     T_{\mathrm{p}}, & \text{$ dvm = 0 $}      
  \end{cases}
\label{eq:xxxxxxxx}
\end{equation}
...
\begin{equation}
 T_{\mathrm{day}} =
  \begin{cases}
     \frac{T_{\mathrm{m}}+T_{\mathrm{p}}}{2}, & \text{$ dvm \neq bottom $ and $ dvm \neq 0 $ } \\
     \frac{T_{\mathrm{b}}+T_{\mathrm{p}}}{2}, & \text{$ dvm = bottom $} \\
     T_{\mathrm{p}}, & \text{$ dvm = 0 $}      
  \end{cases}
\label{eq:xxxxxxx}
\end{equation}
...
During night time, all of them regardless of size are in the pelagic water ($T_{\mathrm{night}}=T_{\mathrm{p}}$).
...
Large demersal fish habitats varies according to the water column conditions.

\begin{equation}
 T_{\mathrm{day}} =
  \begin{cases}
     T_{\mathrm{m}}, & \text{$ (bottom - dvm) < 1500 $ and $ bottom \geq photic $} \\
     T_{\mathrm{b}}, & \text{$ (bottom - dvm) \geq 1500 $ } \\
     \frac{T_{\mathrm{b}}+T_{\mathrm{p}}}{2}, & \text{$ bottom < photic $} 
  \end{cases}
\label{eq:xxxxxx2}
\end{equation}
...
\begin{equation}
 T_{\mathrm{night}} =
  \begin{cases}
     T_{\mathrm{b}}, & \text{$ bottom \geq photic $}  \\
     \frac{T_{\mathrm{b}}+T_{\mathrm{p}}}{2}, & \text{$ bottom < photic $} 
  \end{cases}
\label{eq:xxxxxx3}
\end{equation}
...
\begin{equation}
z = \exp \left( \frac{\ln (w_{\mathrm{max}}) - \ln (w_{\mathrm{min}})}{n} \right)
\end{equation} 
where $w_{\mathrm{min}}$ and $w_{\mathrm{max}}$ are smallest and largest fish in a functional type. There are $n+1$ boundaries of $n$ size bins ($m_{\mathrm{b},i}$), including all lower boundary sizes ($m_{\mathrm{l},i}$) and upper boundary sizes ($m_{\mathrm{u},i}$). 
\begin{equation}
m_{\mathrm{b},i} =  \exp( \ln(w_{\mathrm{min}}) + (i-1) \ln(z)), \hspace{1.5cm}  i\in[1,n+1]
\end{equation} 
\begin{equation}
