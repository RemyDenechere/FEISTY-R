...
\end{equation}
The calculation of $\theta_{s,i,j}$ follows Eq. \ref{eq:lognormalpreference} in setupVertical and Eq. \ref{eq:errorpreference} in setupVertical2. The detail description of $\theta_{v,i,j}$ in the Section **Vertical overlap**.
...
In practical, the feeding preference setting is more complex, since it plays a vital role in coexistence of various functional types. when the feeding preference $\theta_{i,j}$ is initially calculated a function (e.g., Eq. \ref{eq:lognormalpreference}), sometimes it needs to be revised or corrected manually according to specific rules, such as cannibalism, habitats, feeding habits, and visibility. Here I introduce how the feeding preference $\theta_{i,j}$ is processed in setupBasic2.
...
To simplify the temperature input, three temperature input ($T_p$, $T_m$, $T_b$) are required rather than the water column temperature profile in setupVertical2. 
...
Therefore, the temperature for large demersals is defined based on the their time spent in pelagic zone and benthic zone, which is called effective temperature:
\begin{equation} 
