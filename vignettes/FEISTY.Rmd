...
**Small pelagics**\
They always stay in the pelagic water ($T_{day}=T_{night}=T_p$).
...
\label{eq:xxxxxxxx}
\end{equation}
During night time, they are in the pelagic water ($T_{night}=T_p$).
...
\begin{equation}
 T_{day} =
  \begin{cases}
     \frac{T_{m}+T_{p}}{2}, & \text{$ dvm \neq bottom $ and $ dvm \neq 0 $ } \\
     \frac{T_{b}+T_{p}}{2}, & \text{$ dvm = bottom $} \\
     T_p, & \text{$ dvm = 0 $}      
  \end{cases}
\label{eq:xxxxxxx}
\end{equation}

All the large pelagics in non-large sizes (small and medium) are in pelagic water the during daytime ($T_{day}=T_p$).\
During night time, all of them regardless of size are in the pelagic water ($T_{night}=T_p$).
...
At night, small and medium bathypelagics are in the pelagic water ($T_{night}=T_p$), but the habitats of large bathypelagics varies with water column conditions: 
\begin{equation}
 T_{night} =
  \begin{cases}
     T_m, & \text{$ dvm \neq bottom $ and $ dvm \neq 0 $ } \\
...
**Demersals**\
Small demersals are pelagic-living ($T_{day}=T_{night}=T_p$); medium demersals are benthic-living ($T_{day}=T_{night}=T_b$).
Large demersal fish habitats varies according to the water column conditions.

\begin{equation}
 T_{day} =
  \begin{cases}
     T_m, & \text{$ (bottom - dvm) < 1500 $ and $ bottom \geq photic $} \\
     T_b, & \text{$ (bottom - dvm) \geq 1500 $ } \\
     \frac{T_{b}+T_{p}}{2}, & \text{$ bottom < photic $} 
  \end{cases}
\label{eq:xxxxxx2}
\end{equation}
...
$a_e$ & Intercept for mass-specific clearance rate & \multicolumn{4}{c}{70} & $m^2 g^{-b_{e}-1}yr^{-1}$ \\
