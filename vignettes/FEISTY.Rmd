...
The remaining fraction of the available energy ($\nu_{i}$) is invested in growth:
\begin{equation}
\kappa_i = 1 - \psi_i
\end{equation}

Then the growth from the current size class ($i$) to next neighboring size class ($i+1$) can be calculated [@deRoos2008]:
\begin{equation}
 \gamma_i =
  \begin{cases}
...
background mortality rate ($\mu_{\mathrm{b},i}$), and fishing mortality rate ($\mu_{\mathrm{f},i}$): 
\begin{equation}
...
$\theta_\mathrm{A}$ & Large fish preference on forage fish$^d$ & \multicolumn{2}{c}{0.5} & / & / & / \\
...
\multicolumn{7}{l}{\footnotesize $^e$ In B1, $\theta_{\mathrm{A}}$ only poses on medium forage fish (the second size class).
...
One of main objectives of FEISTY model is illustrating the bottom-up mechanisms of fish communities. Here we demonstrate two setups with oligotrophic conditions eutrophic conditions based on `setupVertical2`.
...
In the oligotrophic water, small pelagic fish, midwater predators, and demersal fish cannot survive (panel f), since the resource productions are low and cannot provide enough food for them (panel d).
```{r, fig.width=8, fig.height=7}
sim1=simulateFEISTY(p=p1,tEnd=500)
plotSimulation(sim1)
```

On the contrary, in the eutrophic water, high resource productions can support the existence of all five functional types albeit the water is deep (panel f). All fish have feeding levels that are higher than their critical feeding levels (panel d).
```{r, fig.width=8, fig.height=7}
